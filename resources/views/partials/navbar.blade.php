<!-- Navbar -->
<nav x-data="{ open: false }" class="bg-[#F5F5F5] dark:bg-gray-900 shadow-sm fixed top-0 left-0 w-full z-50 border-b border-[#d2e3ea] dark:border-gray-800">
    ...
    @if (Auth::user()->role_id === \App\Constants\RoleConstant::CUSTOMER)
        <x-dropdown align="right" width="48">
            <x-slot name="trigger">
                <button class="px-3 py-2 rounded-lg text-sm text-gray-700 dark:text-gray-300 hover:bg-[#e0f4f7] dark:hover:bg-[#183c3d] transition focus:outline-none focus:ring-2 focus:ring-[#8ee4d9]">
                    {{ Auth::user()->name }}
...
@else
                ...
                <a href="{{ route('login') }}" class="text-sm px-3 py-2 rounded-lg text-[#188e9c] dark:text-[#7de2d1] hover:bg-[#e0f4f7] dark:hover:bg-[#183c3d] transition focus:outline-none focus:ring-2 focus:ring-[#8ee4d9]">Log in</a>
                <a href="{{ route('register') }}" class="text-sm px-3 py-2 rounded-lg text-[#188e9c] dark:text-[#7de2d1] hover:bg-[#e0f4f7] dark:hover:bg-[#183c3d] transition focus:outline-none focus:ring-2 focus:ring-[#8ee4d9]">Register</a>
            @endauth
        </div>
    ...
    <div x-show="open" x-transition
        ...
        @foreach([
            ...
            <x-responsive-nav-link :href="route($route)" :active="request()->routeIs($route)" class="block py-2 border-b border-[#e0f4f7] dark:border-[#2acfc9] hover:bg-[#daf7f0] dark:hover:bg-[#183c3d] focus:outline-none focus:ring-2 focus:ring-[#8ee4d9]">
                {{ $label }}
    ...
    @if (Auth::user()->role_id === \App\Constants\RoleConstant::CUSTOMER)
        ...
        <x-responsive-nav-link :href="route('pages.wishlist')" class="block py-2 border-b border-gray-200 dark:border-gray-700">{{ __('Wishlist') }}</x-responsive-nav-link>
        <form method="POST" action="{{ route('logout') }}">
            @csrf
