...
<div class="max-w-md mx-auto mt-10 bg-white p-6 rounded shadow border">
    <h2 class="text-xl font-bold mb-6">
        Edit {{ ucfirst($type) }} Image &mdash; {{ $car->brand }} {{ $car->model }}
    </h2>
    ...
    <form action="{{ route('dealer.gallery.update', $gallery->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')

        <div>
            <label class="block font-semibold mb-1">Current Image</label>
            <img src="{{ asset('storage/galleries/' . $gallery->image_path) }}" alt="Current" class="w-full h-48 object-cover rounded">
        </div>
