...
@section('content')
    ...
    <!-- Hero Section -->
    <div class="relative bg-cover bg-center py-20 pt-[120px]"
        style="background-image: url('{{ asset('images/brochure.jpg') }}');">
        ...
        <div class="container mx-auto px-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                @forelse($brochures as $brochure)
                    <div class="brochure-card text-center bg-gray-50 rounded shadow p-3">
                        <img src="{{ asset('storage/' . $brochure->image_path) }}" alt="{{ $brochure->title }}"
                            class="h-[320px] w-auto mx-auto object-contain rounded mb-3">
                        <h3 class="font-semibold text-base text-gray-800 mb-1">{{ $brochure->title }}</h3>
                        <div class="text-sm text-gray-500 italic flex flex-col mb-3 leading-tight">
                            <span>Released in {{ $brochure->month }} {{ $brochure->year }}</span>
                            <span>{{ $brochure->size }} MB</span>
                        </div>
                        <a href="{{ asset('storage/' . $brochure->pdf_path) }}" target="_blank"
                            class="bg-red-600 text-white py-1.5 px-5 text-sm font-semibold rounded hover:bg-red-700 transition duration-200">
                            DOWNLOAD
                        </a>
                    </div>
                @empty
