<div x-data="searchComponent()" x-init="init()" class="relative w-full">
    ...
    >
        <svg class="w-4 h-4 text-blue-600 dark:text-blue-300 mr-1.5 transition-transform duration-300 group-hover:scale-110" viewBox="0 0 24 24" fill="currentColor">
    ...
    <div
        x-show="search.length > 0 && (suggestions.length > 0 || loading || !loading)"
    ...
    >
            ...
            <ul>
                <template x-for="(item, idx) in suggestions" :key="item.id">
                    <li>
                        ...
                        >
                            <div class="w-20 h-12 flex items-center justify-center bg-gray-50 dark:bg-gray-700 rounded-md flex-shrink-0 overflow-hidden shadow">
                            ...
                            <div class="flex flex-col flex-1 min-w-0">
                                <span class="text-gray-900 dark:text-gray-100 font-semibold text-base truncate" x-text="item.brand + ' ' + item.model"></span>
                                <span class="text-gray-500 dark:text-gray-400 text-xs mt-0.5 truncate" x-text="'Year: ' + item.year + ' • ' + item.category"></span>
                                <span class="text-blue-800 dark:text-blue-200 font-bold text-xs mt-0.5" x-text="item.price ? 'Rp ' + (Number(item.price)).toLocaleString('id-ID') : ''"></span>
                            </div>
                            <svg class="w-4 h-4 ml-auto text-blue-400 group-hover:text-blue-700 transition" fill="none" viewBox="0 0 24 24">
        ...
        <template x-if="!loading && suggestions.length === 0 && search">
            <div class="px-6 py-4 text-gray-400 dark:text-gray-500 text-base text-center">
                <svg class="w-6 h-6 mx-auto mb-2" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 13h6m-3-3v6m-7 4a9 9 0 1118 0 9 9 0 01-18 0z"/>
                </svg>
                No results found.
            </div>
        </template>
    ...
    return {
        ...
        async fetchSuggestions() {
            ...
            this.timeout = setTimeout(async () => {
                try {
                    const res = await fetch(`/search-autocomplete?query=${encodeURIComponent(this.search)}`);
