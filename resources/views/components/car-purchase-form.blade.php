...
@if (session('success'))
    <div x-data="{ show: true }" x-show="show" x-transition:enter="transition ease-out duration-300"
        ...
        x-transition:leave-end="opacity-0" class="fixed inset-0 flex items-center justify-center bg-black/70 z-[60]"
        aria-modal="true" role="dialog" x-cloak>
        <div @click.away="show=false"
            ...
            <div class="flex justify-center mb-5">
                <div class="relative flex items-center justify-center h-14 w-14">
                    <span
                        class="absolute inline-flex h-full w-full rounded-full bg-green-100 animate-ping opacity-60"></span>
                    <svg class="w-14 h-14 text-green-500 drop-shadow-lg" fill="none" stroke="currentColor"
            ...
            <p class="text-gray-700 text-sm md:text-base mb-4 leading-relaxed">
                Your order has been received and will be processed within <span class="font-semibold text-green-700">3–7
                    business days</span>.
                We will contact you soon for the next steps.
            </p>
            <div class="mb-6 text-gray-500 text-xs md:text-sm">
                Need help? <a :href="'https://wa.me/6281378535706'" target="_blank"
                    class="text-indigo-600 font-semibold hover:underline">Contact Support</a>
            </div>
            <button
                @click="show = false; @if (session('redirectTo')) window.location.href='{{ session('redirectTo') }}' @endif"
...
<div class="flex justify-center items-start min-h-screen bg-gray-50 py-10 px-4">
    <div class="bg-gray-100 rounded-2xl p-8 w-full max-w-3xl shadow-lg">
        ...
        <form action="{{ route('purchase.submit') }}" method="POST" enctype="multipart/form-data"
            x-data="creditForm({{ $carPrice }})" x-init="init()">
            @csrf
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <x-text-input id="phone" name="phone" type="text" class="mt-1 block w-full"
                        placeholder="0812xxxxxxx" value="{{ old('phone') }}" required />
                ...
                <div>
                    ...
                    <select id="payment_method" name="payment_method" x-model="paymentMethod"
                        class="appearance-none mt-1 block w-full border-gray-300 rounded-md px-3 py-2 text-sm" required>
                ...
                <div class="md:col-span-2">
                    ...
                    <x-textarea id="address" name="address" class="w-full" placeholder="Enter your full address"
                        required />
            ...
            <div class="flex justify-center gap-4 mt-8">
                ...
                @else
                    <x-button type="button" color="black" onclick="window.location='{{ route('login') }}'">
                        Log in to submit
                    </x-button>
                @endauth
            </div>
