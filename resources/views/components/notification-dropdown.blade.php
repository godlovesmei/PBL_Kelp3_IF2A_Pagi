...
<div x-data="{ openNotif: false }" @mouseenter="if (!{{ $mobile ? 'true' : 'false' }}) openNotif = true"
    @mouseleave="if (!{{ $mobile ? 'true' : 'false' }}) openNotif = false" class="relative">
    <button @click="openNotif = !openNotif"
        ...
        :aria-expanded="openNotif" aria-label="Toggle notifications" type="button" @keydown.escape="openNotif = false">
        ...
        @auth
            @if ($unreadCount > 0)
                <span
                    class="absolute top-1 -right-1 bg-red-600 text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center leading-none">
                    {{ $unreadCount }}
                </span>
            @endif
        @endauth
    ...
    <div x-show="openNotif" x-transition @click.outside="openNotif = false"
        class="
            absolute
        ...
        @auth
            <div
                class="p-3 border-b border-gray-200 dark:border-gray-700 font-semibold text-gray-800 dark:text-gray-100 text-base">
                Notifications
            </div>
            ...
            <div
                class="px-4 py-2 text-right border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 rounded-b-lg">
                <a href="{{ route('user.orders.index') }}"
                    class="text-sm text-blue-600 dark:text-teal-400 hover:underline font-medium">
                    View My Orders
                </a>
            </div>
        @else
            <div class="p-4 flex flex-col items-center space-y-2 text-center text-gray-600 dark:text-gray-300">
                ...
                <a href="{{ route('login') }}"
                    class="text-blue-600 dark:text-teal-400 hover:underline font-medium text-sm">
                    Log in
                </a>
            </div>
        @endauth
