...
<form action="{{ $action }}" method="POST" enctype="multipart/form-data"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
        ...
        <div>
            ...
            <input type="number" id="year" name="year" required min="1900" max="{{ $currentYear }}"
        ...
        <div>
            ...
            <select id="category" name="category" required
                class="w-full rounded-md border border-gray-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="" disabled {{ old('category', $car->category ?? '') == '' ? 'selected' : '' }}>--
                    Select Category --</option>
                @foreach ($categories as $cat)
                    <option value="{{ $cat }}"
                        {{ old('category', $car->category ?? '') == $cat ? 'selected' : '' }}>{{ $cat }}
                    </option>
                @endforeach
            </select>
        </div>
                    ...
                    @foreach ($oldGalleries as $idx => $img)
                        <div class="flex items-center gap-4">
                            ...
                            <button type="button" onclick="removeGalleryImage(this)"
                                class="text-red-600 font-bold text-xl px-2">&times;</button>
                        </div>
                    @endforeach
                @else
                    <div class="flex items-center gap-4">
        ...
        <div id="colors-wrapper" class="space-y-4">
            ...
            @forelse ($colors as $index => $color)
                <div class="flex space-x-3 items-center">
                    <input type="text" name="colors[{{ $index }}][color_name]" placeholder="Color name"
                        required
                        value="{{ old("colors.$index.color_name", is_array($color) ? $color['color_name'] : $color->color_name ?? '') }}"
                        class="w-1/3 rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    <input type="text" name="colors[{{ $index }}][hex]" placeholder="#hexcode"
                        pattern="^#([A-Fa-f0-9]{6})$" title="Hex format like #FFFFFF" required
                        value="{{ old("colors.$index.hex", is_array($color) ? $color['hex'] : $color->hex ?? '') }}"
                        class="w-1/3 rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 color-hex-input"
                        oninput="updateColorPreview(this)" />
                    <span class="inline-block w-8 h-8 rounded border ml-1 color-preview"
                        style="background: {{ old("colors.$index.hex", is_array($color) ? $color['hex'] : $color->hex ?? '') }}"></span>
                    <input type="text" name="colors[{{ $index }}][alt_hex]" placeholder="#alt_hex"
                        pattern="^#([A-Fa-f0-9]{6})$" title="Hex format like #FFFFFF" required
                        value="{{ old("colors.$index.alt_hex", is_array($color) ? $color['alt_hex'] : $color->alt_hex ?? '') }}"
                        class="w-1/3 rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 color-hex-input"
                        oninput="updateColorPreview(this)" />
                    <span class="inline-block w-8 h-8 rounded border ml-1 color-preview"
                        style="background: {{ old("colors.$index.alt_hex", is_array($color) ? $color['alt_hex'] : $color->alt_hex ?? '') }}"></span>
            ...
            @empty
                <div class="flex space-x-3 items-center">
                    <input type="text" name="colors[0][color_name]" placeholder="Color name" required
                        class="w-1/3 rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    <input type="text" name="colors[0][hex]" placeholder="#hexcode" pattern="^#([A-Fa-f0-9]{6})$"
                        title="Hex format like #FFFFFF" required
                        class="w-1/3 rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 color-hex-input"
                        oninput="updateColorPreview(this)" />
                    <span class="inline-block w-8 h-8 rounded border ml-1 color-preview"></span>
                    <input type="text" name="colors[0][alt_hex]" placeholder="#alt_hex"
                        pattern="^#([A-Fa-f0-9]{6})$" title="Hex format like #FFFFFF" required
                        class="w-1/3 rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 color-hex-input"
                        oninput="updateColorPreview(this)" />
                    <span class="inline-block w-8 h-8 rounded border ml-1 color-preview"></span>
