<div
    x-data="{
        ...
        resetModal() { this.modalList = []; this.modalIndex = 0; },
        ...
        <div class="flex justify-center mb-10" data-aos="zoom-in">
            <nav class="inline-flex rounded-full shadow-sm bg-gray-100 p-1 relative">
                <button
                    ...
                    aria-label="View exterior images"
                    type="button"
                    data-aos="fade-right"
                >
                ...
                <button
                    @click="galleryTab = 'interior'"
                    :class="galleryTab === 'interior'
                        ? 'bg-gradient-to-r from-red-600 to-black text-white shadow-lg scale-105'
                        : 'text-gray-800 hover:bg-red-50 border border-red-200'"
                    class="transition-all duration-200 px-6 py-2 rounded-full font-semibold focus:outline-none focus:ring-2 focus:ring-red-400 flex items-center gap-2"
                    aria-label="View interior images"
                    type="button"
                    data-aos="fade-left"
                >
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div @keydown.window="handleKey($event)" x-cloak>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
                            <template x-for="(img, idx) in exteriorImages" :key="idx">
                                <div
                                    ...
                                    @click="openModal(exteriorImages, idx, $event)"
                                    :data-aos="'zoom-in-up'"
                                ...
                                >
                                    ...
                                    <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-black/30 transition rounded-xl">
                                        <span class="text-xs font-semibold text-white bg-black/60 px-2 py-1 rounded">View Image</span>
                                    </div>
                                    <!-- Caption -->
                    ...
                    <template x-if="interiorImages.length > 0">
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
                            <template x-for="(img, idx) in interiorImages" :key="idx">
                                <div
                                    class="relative group cursor-pointer transition hover:scale-105"
                                    @click="openModal(interiorImages, idx, $event)"
                                    :data-aos="'zoom-in-up'"
                                    :data-aos-delay="(idx % 3) * 100"
                                >
            ...
            >
                <div class="relative">
                    ...
                    <div class="absolute bottom-3 left-1/2 -translate-x-1/2 w-max flex flex-col items-center gap-1 z-50">
                        ...
                        <div class="flex gap-1 mt-1">
                            <template x-for="(img, dotIdx) in modalList" :key="dotIdx">
                                <span :class="dotIdx === modalIndex
                                    ? 'bg-blue-500'
                                    : 'bg-white/70 hover:bg-blue-300'"
                                    class="inline-block w-2 h-2 rounded-full transition-all ring-1 ring-white mx-0.5"
                                ></span>
                            </template>
                        </div>
                    </div>
                    <button @click="nextImage()"
                        x-show="modalIndex < modalList.length-1"
