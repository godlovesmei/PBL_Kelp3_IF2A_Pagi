    ...
    <div class="mb-6">
        ...
        <div class="flex items-center gap-2">
            <input 
                ...
                min="{{ $minDp }}" 
                max="{{ $maxDp }}" 
                placeholder="Enter Down Payment ({{ $minDp }}-{{ $maxDp }}%)"
...
function showSimulation() {
    ...
    fetch(`{{ url('simulate-price/'.$car->id) }}?dp=${dpPercent}`)
        ...
        .then(data => {
            ...
            for (let i = 1; i <= 5; i++) {
                document.getElementById('installment'+i).innerText = 'Rp' + data.installments[i-1].toLocaleString('id-ID');
                document.getElementById('admin'+i).innerText = 'Rp' + data.admin[i-1].toLocaleString('id-ID');
                document.getElementById('total'+i).innerText = 'Rp' + data.total[i-1].toLocaleString('id-ID');
            }
