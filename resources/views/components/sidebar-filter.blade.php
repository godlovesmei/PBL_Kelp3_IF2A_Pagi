...
<div id="sidebar-overlay"
    class="lg:hidden fixed inset-0 z-40 bg-black/30 backdrop-blur-sm transition-opacity duration-200 opacity-0 pointer-events-none">
</div>

<form action="{{ route('pages.shop') }}" method="GET" id="filter-sidebar"
...
<div>
    <label class="block text-gray-700 dark:text-gray-300 font-medium mb-2">
        <svg class="inline mb-1 mr-1 w-4 h-4 text-indigo-500" fill="none" stroke="currentColor" stroke-width="2"
            viewBox="0 0 24 24">
...
<div>
    <label class="block text-gray-700 dark:text-gray-300 font-medium mb-2">
        <svg class="inline mb-1 mr-1 w-4 h-4 text-green-500" fill="none" stroke="currentColor" stroke-width="2"
            viewBox="0 0 24 24">
        ...
        @foreach ($priceOptions as $value => $label)
            <label for="price-{{ $value }}"
                ...
                <input id="price-{{ $value }}" type="checkbox" name="price[]" value="{{ $value }}"
                    ...
                    {{ in_array($value, request()->get('price', [])) ? 'checked' : '' }} onchange="onFilterChange()">
                <span>{{ $label }}</span>
            </label>
        @endforeach
    </div>
