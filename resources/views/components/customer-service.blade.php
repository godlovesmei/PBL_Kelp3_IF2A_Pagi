<div x-data="{ openChat: false, chatMessage: '' }" class="fixed bottom-5 right-5 z-50">
    ...
    <button
        ...
        @click="openChat = true" aria-label="Open WhatsApp Chat">
        <!-- Clean WhatsApp SVG icon -->
        <svg viewBox="0 0 448 512" width="22" height="22" fill="currentColor" aria-hidden="true" focusable="false"
    ...
    <!-- Chat Popup -->
    <div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[999]" x-show="openChat"
        x-cloak>
        <div class="bg-white w-[92%] max-w-xs rounded-xl p-4 shadow-xl relative transform transition-all duration-300 scale-95"
            x-transition>
            <h2 class="text-base font-semibold mb-2 text-gray-800">Chat with Us</h2>
            <textarea x-model="chatMessage" placeholder="Type your message..."
            ...
            <div class="mt-3 flex justify-end gap-2 text-sm">
                ...
                <a :href="'https://wa.me/6281378535706?text=' + encodeURIComponent(chatMessage)" target="_blank"
